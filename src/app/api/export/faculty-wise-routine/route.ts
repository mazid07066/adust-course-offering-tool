...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const targetFaculty = teacherCode
      ? facultyOptions.filter((item) => item.startsWith(`${teacherCode} -`) || item === teacherCode)
      : facultyOptions;
    ...
    for (const faculty of targetFaculty) {
      ...
      const facultyRows = rows.filter((row) => row.facultyCodes.includes(code));
      const sheet = setupWorksheet(workbook, safeFileName(code).slice(0, 31));
      ...
      sheet.getCell("A2").value = `Faculty-wise Routine — ${faculty}`;
      ...
      const sortedDays = uniqueStrings(facultyRows.map((row) => row.dayOfWeek))
        .filter((day) => day !== "-")
      ...
      for (const day of sortedDays) {
        ...
        for (const row of facultyRows.filter((row) => row.dayOfWeek === day)) {
      ...
      if (!facultyRows.length) {
        const emptyRow = sheet.addRow(["No routine rows found for this faculty."]);
    ...
    if (!targetFaculty.length) {
      const sheet = setupWorksheet(workbook, "Faculty Routine");
      sheet.addRow(["No faculty routine rows found."]);
    }
    ...
    return new Response(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${safeFileName(
          termName
        )}_faculty_wise_routine.xlsx"`,
      },
    });
  } catch (error) {
    console.error("Faculty-wise routine export failed:", error);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to export faculty-wise routine.",
      },
      { status: 500 }
    );
  }
