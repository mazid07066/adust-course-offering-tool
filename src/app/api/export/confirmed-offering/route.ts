  ...
  try {
    ...
    for (const course of courses) {
      ...
      lines.push(
        [
          ...
          course.section,
          Number(course.master_courses.credit || 0),
          course.primary_offered_course_id ? "SECONDARY" : "PRIMARY",
          course.primary_offered_course_id
            ...
            : "-",
          course.offered_course_batches.map((x) => x.batches.batch_code).join(", "),
          facultyText,
          buildScheduleText(effectiveSlots),
        ]
          .map(csvEscape)
          .join(",")
      );
    ...
    return new NextResponse(lines.join("\n"), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="confirmed_offering_${term.name.replace(/\s+/g, "_")}.csv"`,
      },
    });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      { error: "Failed to export confirmed offering report." },
      { status: 500 }
    );
  }
}