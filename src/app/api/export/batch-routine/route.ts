...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const batchFilePart = batchCodeFilter || "ALL_BATCHES";

    return new NextResponse(lines.join("\n"), {
      status: 200,
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="batch_routine_${batchFilePart}_${term.name.replace(/\s+/g, "_")}.csv"`,
      },
    });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      { error: "Failed to export batch routine." },
      { status: 500 }
    );
  }
}