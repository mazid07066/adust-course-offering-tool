...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const studentIdSuffixRaw = normalizeText(body.studentIdSuffix);
    const studentIdSuffix = studentIdSuffixRaw
      ? normalizeUpper(studentIdSuffixRaw)
      : null;
    ...
    if (!curriculumVersion) {
      return NextResponse.json(
        { error: "Curriculum version is required." },
        { status: 400 }
      );
    }
    ...
    console.error("Academic setup POST error:", error);
