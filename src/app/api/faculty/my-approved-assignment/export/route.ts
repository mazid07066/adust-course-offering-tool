...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const ws = workbook.addWorksheet("Approved Assigned Schedule");
    ...
    ws.getCell(`A${row}`).value = "Approved Assigned Schedule";
    ...
    ws.getCell(`E${row}`).value = "Date and time of export:";
    ws.getCell(`F${row}`).value = new Date().toLocaleString();
    row += 1;
    ...
    if (scheduleRows.length === 0) {
      ws.getRow(row).values = ["No approved assigned courses found."];
      row += 1;
    }
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="approved_assigned_schedule_${teacher.teacher_code}_${term.name.replace(/\s+/g, "_")}.xlsx"`,
      },
    });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      { error: "Failed to export approved assigned schedule." },
      { status: 500 }
    );
  }
}