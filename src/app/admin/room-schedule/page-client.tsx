...
export default function RoomSchedulePageClient() {
  ...
  async function loadReport(selectedRoomCode?: string) {
    if (!termName) return;
    ...
    try {
      ...
      if (selectedRoomCode || roomCode) {
        qs.set("roomCode", selectedRoomCode || roomCode);
      }

      const res = await fetch(`/api/admin/reports/room-schedule?${qs.toString()}`, {
        cache: "no-store",
      });
      ...
      if (!res.ok) {
        throw new Error(json.error || "Failed to load room schedule report.");
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to load room schedule report.");
      setRoomOptions([]);
      setRows([]);
      setSummary(null);
    } finally {
  ...
  return (
    <AdminLayout title="Room Schedule Report">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="flex items-end gap-3">
              ...
              >
                {loading ? "Loading..." : "Refresh Room Schedule"}
              </button>

              <a
                href={`/api/export/room-schedule?termName=${encodeURIComponent(termName || "")}&roomCode=${encodeURIComponent(roomCode || "")}`}
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr
                  key={`${row.roomCode}-${row.courseCode}-${row.section}-${row.dayOfWeek}-${row.startTime}-${index}`}
                >
              ...
              {rows.length === 0 && !loading && (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-slate-500">
                    No confirmed room schedule rows found.
                  </td>
                </tr>
              )}
            </tbody>
